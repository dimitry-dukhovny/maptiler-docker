...
[ "${DOCKER}" = "NONE" -o "${DOCKERCOMPOSE}" = "NONE" ] && \
  nope "Install docker and docker-compose before resuming."
[ "${STUNNEL}" = "NONE" ] && nope "Install stunnel before resuming."

groupadd docker 2> /dev/null
getent group docker > /dev/null 2>&1 || \
  nope "Could not create the docker group.  Are you root?"
useradd -d /home/svc.map -m -s /bin/bash -G docker svc.map 2> /dev/null
getent passwd svc.map > /dev/null 2>&1 || \
  nope "Could not create the svc.map user.  Are you root?"
...
[ ! -d /etc/letsencrypt/live/default ] && \
  ln -s ${assumedcert} /etc/letsencrypt/live/default
cp -f ${mydir}/stunnel.service /usr/lib/systemd/system/
[ -f /etc/stunnel/stunnel.conf ] && \
  mv /etc/stunnel/stunnel.conf /etc/stunnel/stunnel.conf.`date +%s`
cp -f ${mydir}/stunnel.conf /etc/stunnel/

# Selinux context
chcon -u system_u -r object_r -t system_unit_file_t \
  /usr/lib/systemd/system/docker-maptiler.service \
  /usr/lib/systemd/system/stunnel.service \
  2> /dev/null
# Ownership
chown -R svc.map:docker /data/map || \
  nope "Failed to change ownership of /data/map!  Not continuing!"
...
echo "Download maps to /data/map."
echo "Adjust /data/map/bin/docker-compose.yml accordingly!"
